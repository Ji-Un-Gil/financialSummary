...
files = ['AGENTS.md', 'docs/editorial-policy.md', 'docs/daily-run.md', 'templates/daily.md', 'templates/evidence.md']
...
금융위원회·금융감독원·한국은행과 관련 해외 공식 발표 및 국내 언론을 조사한다. 검증한 소식만 3~5건 이내로 작성하며 부족하면 줄인다. 원문에 근거한 초보자 설명과 자체 표·도식, 출처 링크를 포함한다. 자료 확보가 불충분하면 부분 수집으로 표시한다. 확인 가능 항목이 없으면 기사 수치를 만들지 말고 수집 상태·실제 시도·한계를 보고서와 검증표로 남긴다. 실제 도구 시각을 얻을 수 없으면 확인 시각을 미상으로 기록한다.
